...
A hash table provides O(1) average-case time complexity for insertion, search, and deletion operations, making it ideal for database management systems.
...
Maps student roll numbers to table indices using modulo operation.

### Linear Probing

Open addressing technique that resolves collisions by searching sequentially for the next available slot.
...
- **2 (Deleted)**: Previously occupied, marked for reuse
...
2. Linear probe until empty/deleted slot found
3. Insert student record
4. Mark state as full
```
...
3. Return index if roll number matches
