# Product Inventory Management with Deletion Operations using BST

## Name: Likhit Chirmade, Roll no: 23

## Theory

### BST for Product Management

Tree organized by product name with deletion capabilities for inventory management.
...
**Algorithm:**
```
1. Search for product by code (not name)
2. Three cases:
   a. Leaf node → Delete directly
   b. One child → Replace with child
   c. Two children → Replace with inorder successor
```

**Inorder Successor:** Minimum node in right subtree

#### Delete Expired Products

**Algorithm:**
```
1. Recursively traverse entire tree
2. For each node:
   a. Process left subtree
   b. Process right subtree
   c. Check if current node expired
   d. If expired → Delete using code
```

**Date Comparison:** String comparison works for YYYY-MM-DD format

### Example

```
Products:
- Apple (code: 101, expiry: 2024-01-15)
- Banana (code: 102, expiry: 2024-12-30)
- Cherry (code: 103, expiry: 2024-01-10)

Current date: 2024-02-01

Delete expired:
- Cherry deleted (expired)
- Apple deleted (expired)
- Banana remains
```
...
|-----------|---------|-------|
| Insert | O(log n) | O(n) |
| Delete by Code | O(n) | O(n) |
| Delete Expired | O(n) | O(n) |
| Display | O(n) | O(n) |

Note: Delete by code is O(n) because search is by code, not by BST key (name)
...
int main() {
    ...
    while (true) {
        cout << "\n--- Product Inventory Management (lac) ---\n";
